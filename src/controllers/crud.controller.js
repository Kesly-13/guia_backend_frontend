...
class CrudController {
    ...
    async obtenerTodos(tabla) {
        try {
            ...
            return resultados; // Devuelve un array con todos los resultados
        } catch (error) {
    ...
    async obtenerUno(tabla, idCampo, id) {
        try {
            // Consulta para obtener un único registro, se pasan nombres de tabla/campo, y un ID
            const [rows] = await db.query('SELECT * FROM ?? WHERE ?? = ?', [tabla, idCampo, id]);
...
try {
            ...
            return { id_creado: resultado.insertId }; // ID generado automáticamente
        } catch (error) {
            throw error;
        }
    }
    ...
    async actualizar(tabla, idCampo, id, data) {
        try {
            ...
            // Si se actualizó al menos un registro, el registro no existía
            if (resultado.affectedRows === 0) {
                throw new Error('Registro no encontrado');
            }

            // Devuelve el registro actualizado
            return this.obtenerUno(tabla, idCampo, id);
        } catch (error) {
            throw error;
        }
    }
    ...
    async eliminar(tabla, idCampo, id) {
        try {
            ...
            // Si no se eliminó ningún registro, el registro no existe
if (resultado.affectedRows === 0) {
                throw new Error('Registro no encontrado');
            }
